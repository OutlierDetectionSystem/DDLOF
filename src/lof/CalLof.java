...
public class CalLof {
	...
	public static class CalLOFMapper extends Mapper<LongWritable, Text, IntWritable, Text> {
		 ...
		 * dist) output format: (Core area)key: partition id || value: nid,
		 * type(S or C), LRD, whoseSupport, (KNN's nid and dist) (Support
		 * area)key: partition id || value: nid, type(S or C), LRD
		 * 
		 * @author yizhouyan
		 */
		...
		public void map(LongWritable key, Text value, Context context) throws IOException, InterruptedException {
			...
			interValue.set(nid + ",C," + lrd + "," + strValue[2] + "," + knn_id_dist);
	...
	public static class CalLOFReducer extends Reducer<IntWritable, Text, LongWritable, Text> {
		...
		private MetricObject parseObject(int key, String strInput) {
			...
			int offset;
			if (type != 'S') {
				offset = inputSplits[0].length() + inputSplits[2].length() + inputSplits[3].length() + 5;
				KNN = strInput.substring(offset, strInput.length());
				whoseSupport = inputSplits[3];
			}
			return new MetricObject(partition_id, obj, lrd, KNN, type, whoseSupport);
		}
		...
		public void reduce(IntWritable key, Iterable<Text> values, Context context)
			...
			for (MetricObject o_S : coreData) {
				hm_lrd.put(((Record) o_S.getObj()).getRId(), o_S.getKdist());
			}
		...
		private void CalLOFForSingleObject(Context context, MetricObject o_S, HashMap<Long, Float> hm)
			...
			if (o_S.getKdist() == 0)
				...
				for (int i = 0; i < splitKNN.length; i++) {
					...
					float temp_lrd = hm.get(Long.valueOf(tempSplit[0]));
					if(temp_lrd == 0 || o_S.getKdist() ==0)
						lof_core = lof_core;
					else
						lof_core += temp_lrd / o_S.getKdist() * 1.0f;
				}
	...
	public void run(String[] args) throws Exception {
		...
		/** set job parameter */
		Job job = Job.getInstance(conf, "DDLOF: Calculate lof");
